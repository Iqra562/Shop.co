...
function WishlistCard({isSidebar,onClose}) {
  ...
  return (
    <>
    <div className={`grid grid-cols-1 gap-x-3  ${isSidebar ? "w-full h-[70vh]  overflow-y-auto  space-y-1 gap-y-2 " :"gap-y-5 "} `}>
...

    {
      ...
      <div className="flex  w-full h-full">
        ...
        <div className={`${isSidebar ? 'pl-3' : 'pl-2 md:pl-8 '} w-full flex flex-col justify-between `}>
          <div className="flex justify-between  w-full items-center">
            <h2 className={`${isSidebar ? 'text-xl' :'text-2xl'}  font-bold`}>{product.name} </h2>
          ...
          <div className="flex justify-between">
            <h3 className={`${isSidebar ? 'text-base':' text-xl'} font-bold text-secondary`}>${product.price}</h3>
            <div>
              <div className="space-x-1 flex items-center">
                {!isSidebar && <AddToCart classAttributes='bg-primary-button-gradient w-fit px-2 py-1 text-xs font-bold text-white uppercase rounded-md' productId={product._id}/>} </div>
            </div>
          </div>
        </div>
      </div>

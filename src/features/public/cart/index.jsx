...
const { mutate: cartRequest, isPending: addToCartLoader } = useMutation({
  ...
  const increaseQuantity = (productId,cartItemId) => {
    ...
    const newQuantity  = Math.min((quantities[cartItemId] || 1) + 1, 15);
     ...
     cartRequest({ productId: String(productId), quantity:newQuantity },{
      onSuccess: () => {
...
{cart.length ===0 ? (
  ...
  <div className="   bg-gray-50 px-20 py-10 rounded-lg w-fit mx-auto">
        ...
        <p className="text-lg  text-center"> <Link to='/products' className=""><span> continue the shopping.. </span>  </Link> </p>
      </div>
      </div>
):(
  <div className="flex pt-10">
                ...
                <div className="flex justify-between">
                  ...
                  <div>
                    <div className="space-x-1 flex items-center">
                      ...
                      <span className="font-bold">{quantities[item._id] > 9 ? '' :0}{quantities[item._id]} </span>
                      <button
        ...
        <div className="pl-8 space-y-14 w-5/12 sticky top-44  self-start">
 ...
 <div className="h-[80vh] flex flex-col items-center justify-center">
        ...
        <h1 className="text-3xl font-bold text-center">You are not loggedin</h1>
        <p className="text-xl  text-center">Please <Link to='/login ' className="underline"><span> sign in </span>  </Link> to check your cart!</p>
      </div>
